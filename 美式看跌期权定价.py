...
for i in range(N,0,-1):
    for j in range (i,0,-1):
        if i == N:  #在到期日计算到期日前一个时间周期
            lattice[i-1][j-1] = max(discount * (p*(poo(lattice[i,j]))+(1-p)*(poo(lattice[i][j-1]))), (50-lattice[i-1][j-1]))
        else:
            lattice[i-1][j-1] = max(discount * (p*lattice[i, j] + (1-p)*lattice[i][j - 1]),50-(lattice[i-1][j-1]))  #非到期日计算
